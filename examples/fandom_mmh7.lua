...
function scrapFaction(page, ctx)
   for creature in page:select("div.tabber table td a"):iter() do
...
function scrapCreature(page, ctx)
   ...
   rec:pushField(row["Upgraded"])
   rec:pushField(row["Size"])
   rec:pushField(row["Attack type"])
   rec:pushField(row["Dwelling"])
   rec:pushField(row["Cost per unit"])
   ...
   rec:pushField(row["Speed"])
   rec:pushField(row["Range"])
   rec:pushField(row["Morale"])
   rec:pushField(row["Destiny"])
   ctx:sendRecord(rec)
end
